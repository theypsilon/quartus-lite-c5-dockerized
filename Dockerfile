...
FROM ubuntu:14.04 AS quartus_base
...
FROM quartus_base AS quartus_slim
COPY --from=quartus_builder /opt/intelFPGA_lite/quartus /opt/intelFPGA_lite/quartus
COPY --from=quartus_builder /opt/intelFPGA_lite/licenses /opt/intelFPGA_lite/licenses

FROM quartus_base AS quartus_heavy
